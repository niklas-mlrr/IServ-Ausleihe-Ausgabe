...
| Pfad | Inhalt |
|------|--------|
| `server/` | FastAPI-App: HTTPS (`tls.py`), WebSocket-Hub (`hub.py`), Rollen/Sessions (`sessions.py`, `state.py` — `AppState` + seit Welle 5 ausgelagert: `RuntimeSettings`, `IservCaches`), IServ-Read-Client (`iserv_client.py`), Druck (`printing.py`), Bücher-Reihenfolge (`book_order.py`), Rate-Limiting (`ratelimit.py`), Scan-Station-Zettel (`scan_station.py` — Code-39-Barcode + A4-PDF), Endpunkte (`routes/`-Paket, `api.py` als Aggregator, `ws.py` für WebSockets) |
| `automation/` | Playwright-Worker (`worker.py`) + Spikes + E2E-Skripte (`e2e_*.py`); Ausgaben in `automation/out/` (gitignored) |
| `web/` | Statische UI ohne Build-Step: `host.html` (Host, schlankes Grundgerüst) + `host.js`/`host.css` (Logik/Styles), `common.js` (gemeinsame Helfer: `escapeHtml`, `isBookDone`, `Beeper`, `connectWebSocket`, Scan-Ansicht: `renderBookRows`/`renderBookAlert`/`statusAlertClass`), `scan-view.css` (gemeinsames Aussehen des Scanmodus von Schülerclient + Scan-Station), `scan.html`/`scan.js` (Helfer-Scanner, Modus A), `student.html`/`student.js` (Schüler, Modus B), `scan-station.html`/`scan-station.js` (Scan-Station für Schüler ohne Handy), `qr-display.html` (iPad), `html5-qrcode.min.js`, `beep.mp3` |
