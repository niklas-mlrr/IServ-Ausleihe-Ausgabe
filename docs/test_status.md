...
>
> Stand: 2026-08-12 (Scan-Station `/scan-station` — Zettel mit Code-39-Barcode
> für Schüler ohne Handy, Scanmodus geteilt mit dem Schülerclient inkl.
> Blockierverhalten, Stationswechsel per Zettel-Code, Host-Queue-Status
> „Bücher sammeln" + Stationsname-Badge, Now-Serving-Stationszeile mit
> Zettel-Code + Stations-Trennen, Rangfolge vor dem Worker-Pool,
...
> Speichern, oder im Serverlauf nie per WS verbunden — Letzteres erst ab
> 5 min Serverlaufzeit), Bestand-leer-/
...
> 5-Sekunden-Highlight — 590 Tests grün.
...
| V53 | **Host-Fortschrittsbaseline übersteht Seiten-Reload:** `loaned_at_load` (Baseline der session-bezogenen X/Y-Anzeige in der Host-Queue) wurde bislang bei jedem WS-Reconnect (Seiten-Reload) neu auf den aktuellen Ausleihstand gesetzt. Neuer Parameter `reset_baseline` an `init_book_progress`/`hydrate_student_info`: nur der echte „Aufrufen"-Pfad (`assign_student_to_helper` → `load_and_push_helper_student`/`load_and_push_paired_student`) setzt die Baseline neu; `ws_scanner`-/`ws_student`-Reconnect und `repush_booklist` übergeben `reset_baseline=False` und lassen sie stehen (`done_isbns` wird trotzdem aus IServ aufgefrischt). | `tests/test_queue_progress.py::test_reconnect_preserves_baseline_but_refreshes_done` + `uv run pytest -q` + `uvx ruff check server/ automation/ tests/` | 2026-08-06 | **395 Tests grün**, Ruff clean; rein serverseitige RAM-State-Logik, kein IServ-Schreibzugriff |
...
| V64 | **Modus-B-Session überlebt ein ausgeschaltetes Handy:** Schüler-Token liegt in `localStorage` statt `sessionStorage` (inkl. Migration), `clearToken()` räumt beide Ablagen (bei `closed`/Code 4006); Close-Code 4009 beendet den übernommenen Tab statt eines Reconnect-Pingpongs zwischen zwei Tabs, die sich den Token jetzt teilen können. Serverseitig neues Feld `StudentSessionB.disconnected_at`; die TTL-Regeln stecken in der reinen Funktion `expired_student_sessions()`: verbundene gepairte Sessions verfallen nicht mehr (offener Socket = Liveness-Beweis, tote Sockets schließt Uvicorns WS-Ping/Pong nach ~40 s), das TTL läuft erst ab dem Verbindungsverlust, Fallback `last_activity`. `PAIRED_IDLE_TTL_S` bedeutet damit „gepairt **und getrennt**", Default 900 → 1800 s. | `tests/test_session_ttl.py` (neu, 5 Fälle: verbunden/still, kurze Trennung, Trennung über TTL, Fallback ohne `disconnected_at`, pending nach Alter) + `uv run pytest` (468 Tests) + `uvx ruff check server/ automation/ tests/` + `node --check web/student.js` | 2026-08-11 | **468 Tests grün** (463 → 468), Ruff und JavaScript-Syntaxprüfung grün. **Live-Check (Handy mitten im Vorgang aus- und wieder einschalten, zweiter Tab am selben Gerät) noch offen**. |

| V65 | **Scan-Station (`/scan-station`) — Zettel mit Barcode für Schüler ohne Handy:** eigenes Bau-Modul `server/scan_station.py` (selbst implementierter Code-39-Encoder + A4-Zettel via PyMuPDF, Barcode exakt 6,5 × 1,2 cm oben rechts, Nummer darunter, Klasse/Name oben links, „Noch vorgemerkt" mit Abhak-Kästchen + „Bereits ausgeliehen"); vierstellige Codes stabil pro Schüler und **nie recycelt**; Station spiegelt den Drucker-Display-Pairing-Fluss (Token in der URL, Registrierungs-Code, Freischaltung per Name, Light/Dark) und scannt Bücher über dasselbe read-only `process_scan`; strenge Annahmeregeln (`resolve_station_code`) + 10 Code-Versuche/Minute; 30-s-Leerlauf zählt erst ab `worker_ready`; Druck läuft als `PrintJob.kind="station_sheet"` durch die normale Druckerwarteschlange, setzt aber **keinen** Leihschein-Status. Zusätzlich: `WorkerPool` bekommt eine echte Warteschlange mit Rangfolge **Helfer → Station → Schülerclient** (FIFO je Rolle, Direktzuteilung statt `notify_all`), `stats()["waiting"]` und `worker_waiting`-Positionsmeldung an wartende Clients. | `tests/test_scan_station.py` (neu, 41 Fälle: Barcode-Maße im erzeugten PDF nachgemessen, Code-Vergabe/Recycling, Endpunkte + Auth-Guards, Annahmeregeln, Leerlauf-TTL, Druckauftrag, „Zettel markiert nie den Leihschein"), `tests/test_ws_scan_station.py` (neu, 15 Fälle: WS-Fluss end-to-end inkl. Rate-Limit, Code-nicht-im-Log, Disconnect gibt Schüler frei), `tests/test_worker_pool.py` (4 neue Rangfolge-Fälle) + `uv run pytest` (528 Tests) + `uvx ruff check server/ automation/ tests/` + `node --check web/*.js` | 2026-08-11 | **532 Tests grün** (468 → 532), Ruff und JavaScript-Syntaxprüfung grün. **Live-Check offen** (s. unten). |
| V66 | **Scanmodus der Station = Scanmodus des Schülerclients:** gemeinsames Aussehen in `web/scan-view.css` (aus `student.html` herausgezogen) und gemeinsame Logik in `web/common.js` (`renderBookRows`, `renderBookAlert`/`hideBookAlert`, `ALERT_META`, `statusAlertClass`, `OK_SCAN_STATUSES`/`BLOCKING_SCAN_STATUSES`) — beide Seiten teilen obere Leiste, Statuszeile samt Farbregeln, Namenszeile, Bücher-Tabelle mit FLIP-Animation und Buch-Hinweis-Modal. Zusätzlich Eingabeart **Kamera/Manuell** wie im Helferclient (Eingabefeld in `#reader`, Enter-Knopf, Fokus-Warnbanner), lokal gemerkt und vom Host per `POST /api/scan-station/input-mode` überschreibbar (Schieberegler „Manuell" neben „Dunkel"). Bugfix: Der `finally`-Block des Stations-WS löst die `ws`-Referenz jetzt vor dem Freigeben des Schülers — vorher wurde auf den toten Socket gesendet und das Aufräumen von Schüler/Worker verzögert. | `tests/test_scan_station.py` (4 neue Fälle zur Eingabeart), `tests/test_ws_scan_station.py` (deterministisches Warten auf das serverseitige Aufräumen, 5× wiederholt grün) + `uv run pytest` (532 Tests) + `uvx ruff check server/ automation/ tests/` + `node --check web/*.js` + automatischer Abgleich, dass keine CSS-Klasse von `student.html` beim Herausziehen verloren ging | 2026-08-11 | **532 Tests grün** (528 → 532), Ruff und JavaScript-Syntaxprüfung grün. **Browser-Livecheck beider Clients offen** (s. unten). |

| V67 | **Scan-Station: Blockierverhalten wie am Handy + Stationswechsel per Zettel-Code:** `ScanStationSession.book_alert_open` — ausgemustertes/anderweitig verliehenes Buch öffnet ein blockierendes Modal ohne eigenen Schließen-Weg (Kamera-Scan UND manuelle Eingabe werden ignoriert, das Feld selbst bleibt aber bedienbar — weitere Codes eintippen/Enter geht, wirkt nur nicht), nur der Host gibt per `/api/clear-book-alert` frei (jetzt auch für Stationen, `AppState.find_station_by_student` neu). Ersatzansprüche bleiben unverändert ausschließlich am Host (kein Code nötig — `process_scan(source="student")` liefert `loaned_to` grundsätzlich nicht an Station/Handy). Jeder Scan während einer laufenden Anmeldung wird zuerst geprüft, ob er der Zettel-Code eines ANDEREN Schülers ist — der Treffer-Check läuft unabhängig von Länge/Ziffernform (reiner Dict-Lookup, nach einem Live-Fund korrigiert: eine anfängliche 4-stellig-Vorprüfung ließ echte Treffer je nach Scanner-Rückgabeformat zu Unrecht als „Buch unbekannt" durchfallen), nur das Rate-Limit (10/Minute) bleibt an die 4-stellige Form gebunden. Trifft ein Treffer zu, wechselt die Station sofort statt „Buch unbekannt" zu melden; scheitert der Wechsel, bleibt der aktuelle Schüler unangetastet. | `tests/test_scan_station.py` (`find_station_by_student`, `/api/clear-book-alert` löst Stationen + zielt nur auf die passende), `tests/test_ws_scan_station.py` (Blockade hält über einen ignorierten Scan hinweg, Host-Freigabe schaltet frei, erfolgreicher/gescheiterter Stationswechsel, zufällig 4-stelliger unbekannter Barcode bleibt `unknown_book`) + `uv run pytest` (541 Tests) + `uvx ruff check server/ automation/ tests/` + `node --check web/*.js` | 2026-08-11 | **541 Tests grün** (532 → 541), Ruff und JavaScript-Syntaxprüfung grün. Zwei Korrekturen nach Live-Test: Eingabefeld war fälschlich `disabled` statt nur ignoriert (behoben), Stationswechsel-Treffer war zu streng an „exakt 4-stellig" gebunden (auf unabhängigen Lookup umgestellt). **Browser-Livecheck weiterhin offen** (s. unten). |

| V68 | **Host-Queue-Status „Bücher sammeln" + Stationsname-Badge:** Zettel-Druck ist das „Aufrufen" des Zettel-/Stations-Flusses (`print_station_sheet_for` aktiviert wartende Schüler, füllt den Fortschritt via `init_book_progress`). `QueueStudent.station_zettel_printed` (persistiert über Ab-/Anmelden hinweg, nur „Status zurücksetzen"/„Trennen" löscht sie) steuert zusammen mit dem Live-Stationsstatus die Host-Statuszeile: „Bücher sammeln" solange nicht an einer Station angemeldet und nicht alles ausgeliehen; an einer Station immer zweizeilig „X/Y ohne Mjb" + „X/Y gesamt" (unabhängig von Vorbestand); abgemeldet zurück zu „Bücher sammeln", außer alles ist ausgeliehen (bleibt bei der Zahl). Stationsname als Badge wie beim Helfer (Now-Serving + Klassen-Queue), Host-Symbol `ICO_HOST` statt Helfer-Symbol (dasselbe SVG wie `ICO_LAPTOP` im Drucker-Display); `AppState._queue_student_as_dict` löst `station_name` auf (Spiegel von `assigned_helper_name`). | `tests/test_scan_station.py` (Zettel-Druck aktiviert wartende Schüler, reaktiviert keinen fertigen, „Status zurücksetzen" löscht die Markierung, `station_name` im Queue-Snapshot mit/ohne Anmeldung) + `uv run pytest` (547 Tests) + `uvx ruff check server/ automation/ tests/` + `node --check web/*.js` | 2026-08-11 | **547 Tests grün** (541 → 547), Ruff und JavaScript-Syntaxprüfung grün. **Browser-Livecheck der neuen Statustexte/Badges offen** (s. unten) — insbesondere die zweizeilige Anzeige ohne Vorbestand (session- und gesamt-Zahl identisch) und der Übergang „Bücher sammeln" ↔ Zahl beim Ab-/Anmelden. |

| V69 | **Now-Serving-Stationszeile mit Zettel-Code + eigenständigem Stations-Trennen:** eigene Zeile im "Aktuell in Ausgabe"-Kästchen direkt unter dem Namen — links Stationsname + Symbol + kleiner Trennen-Knopf (grau, nur solange gerade angemeldet), rechts der vierstellige Zettel-Code (blau, sobald überhaupt gedruckt). `QueueStudent.as_dict()` führt `station_code`, aber NUR wenn `include_station_code=True` explizit gesetzt wird — der bestehende `_queue_student_as_dict`/`queue_as_list` ist auch der Serialisierungsweg für die Helferclient-Queue-Pfade (`pending_queue_as_list`/`real_contexts_summary`), dort bleibt der Code weiterhin weg (Credential, PLAN §3.7); nur die beiden `state_snapshot()`-Aufrufe (Host) setzen das Flag. Neuer Endpoint `POST /api/scan-station/release-student` (Spiegel von `/api/scan-station/release`, vom Schüler statt der Station aus adressiert — das Now-Serving-Kästchen kennt nur die `student_id`) löst NUR die Stationsbindung, der Schüler bleibt sonst unverändert aktiv. | `tests/test_scan_station.py` (`station_code` nur mit `include_station_code=True`, nie in den Helferclient-Pfaden, im echten `state_snapshot()` an beiden Stellen; neuer Endpoint inkl. Host-Auth-Gate, fehlendem `student_id`, Idempotenz ohne Station) + `uv run pytest` (556 Tests) + `uvx ruff check server/ automation/ tests/` + `node --check web/*.js` | 2026-08-11 | **556 Tests grün** (547 → 556), Ruff und JavaScript-Syntaxprüfung grün. **Browser-Livecheck offen** (s. unten) — insbesondere die Farb-/Ausrichtungsgestaltung (grau links/blau rechts) und der Stations-Trennen-Knopf am echten Gerät. |

| V70 | **Scan-Station: Leerlauf-Timer während blockierendem Buch-Hinweis ausgesetzt:** solange `ScanStationSession.book_alert_open` ein blockierendes Modal offen hält (ausgemustert/anderweitig verliehen, wartet auf `/api/clear-book-alert`), zählt das 30-s-Leerlauf-TTL nicht mehr — weder serverseitig (`sessions.expired_scan_stations` schließt `book_alert_open`-Stationen aus) noch clientseitig (`freezeIdle`/`unfreezeIdle` in `web/scan-station.js` stoppen/starten den lokalen Countdown-Timer). Der Timer startet erst mit der Host-Freigabe wieder frisch bei vollen 30 s (`queue.clear_book_alert` setzt `station.last_activity` beim Freigeben neu, statt die stillstehende Zeit sofort nachzuholen; `unfreezeIdle` ruft `startIdle()`, was ebenfalls neu von vorn zählt). Nicht-blockierende, selbst schließbare Meldungen (`dismissible=true`) setzen `book_alert_open` nie — der Timer läuft für sie unverändert weiter. | `tests/test_scan_station.py` (`test_idle_sweep_ignores_stations_with_open_book_alert`, `test_clear_book_alert_unblocks_station` um Reset-Assertion erweitert) + `uv run pytest` (557 Tests) + `uvx ruff check server/ automation/ tests/` | 2026-08-12 | **557 Tests grün** (556 → 557), Ruff clean. **Browser-Livecheck offen** (s. unten) — insbesondere, dass der sichtbare Countdown während der Meldung wirklich pausiert statt weiterzuzählen.

| V71 | **Host: Zettel-Nachdruck-Knopf im „Aktuell in Ausgabe"-Kästchen:** neuer Icon-Knopf rechts neben „Trennen" (`data-action="reprint-station-sheet"`, sichtbar sobald `s.station_code` gesetzt ist — dieselbe Bedingung wie die Code-Anzeige links), öffnet denselben Druck-Dialog wie der ursprüngliche Zettel-Druck aus der Klassen-Queue (`printStationSheet()` wiederverwendet, kein neuer Server-Endpoint). Neues `ICON_SHEET` — Blatt mit Eselsohr oben links (gespiegelt gegenüber `docLetterIcon` in `scan-render.js`, das oben rechts knickt), Barcode oben rechts davon, darunter zwei gleich lange Listenzeilen mit Abstand vorm Häkchen (nur unten). Höhe deckungsgleich mit `ICON_ACTION_CHECK`/`ICON_PRINTER` (per `getBBox()` geprüft), Breite bewusst schmaler im A4-Seitenverhältnis (~1:1,43). Vier Korrekturrunden nach Niklas' Feedback, jede per Playwright-Screenshot gegengeprüft. **Server-Korrektur** dabei aufgedeckt: `print_station_sheet_for` hatte `reset_baseline` hart auf `True` verdrahtet — korrekt für das echte „Aufrufen" (`pending → active`), aber der neue Knopf ist der erste tatsächliche Aufrufer dieses Pfads für einen bereits AKTIVEN Schüler, wo das die laufende „seit Aufrufen"-Fortschrittszählung (`loaned_at_load`) der Host-Statuszeile unbeabsichtigt zurückgesetzt hätte. Jetzt nur noch bei `status == "pending"` gesetzt. | `tests/test_scan_station.py::test_print_sheet_reprint_for_active_student_keeps_baseline` (neu — `loaned_at_load` bleibt bei Nachdruck für einen aktiven Schüler unverändert), bestehende `pending`/`done`-Fälle weiterhin grün (unveränderte Semantik) + `uv run pytest` (558 Tests) + `uvx ruff check server/ automation/ tests/` + `node --check web/*.js` | 2026-08-12 | **558 Tests grün** (557 → 558), Ruff und JavaScript-Syntaxprüfung grün. **Browser-Livecheck offen** (s. unten) — insbesondere Icon-Darstellung neben „Trennen" und dass ein Nachdruck den sichtbaren Host-Fortschritt eines aktiven Schülers nicht sichtbar zurücksetzt.

| V72 | **Host: „Erstellen" statt „Drucken" im Pairing-Kasten — Aktivieren ohne Zettel-Druck möglich:** Der Knopf neben der Schüler-Auswahl in der „Scan-Station:"-Zeile heißt jetzt „Erstellen" statt „Drucken" (`renderCtxStationPrint`) — er öffnet weiterhin denselben Druck-Dialog, der aber im Zettel-Modus jetzt DREI Optionen bietet: „Erstellen und Drucken" (bisheriges Verhalten, primary), „Erstellen" (neu, secondary — aktiviert Schüler + vergibt Zettel-Code + befüllt Fortschritt, baut aber KEIN PDF und druckt nicht), „Abbrechen" (ghost, unverändert). Neuer Server-Endpoint `POST /api/scan-station/activate` + `sessions.activate_station_student()`, der sich die Aktivierungslogik mit `print_station_sheet_for` über den neuen gemeinsamen Kern `_load_and_activate_station_student()` teilt (keine Verhaltens-Divergenz zwischen Druck- und Nur-Erstellen-Pfad). Der physische Druck kann jederzeit später über den Zettel-Nachdruck-Knopf im „Aktuell in Ausgabe"-Kästchen (V71) nachgeholt werden — derselbe Zettel-Code bleibt dabei stabil, nur die Bücherliste wird pro Druck aktualisiert (V73, nach kurzem Zwischenstand mit wechselndem Code in V74 wieder korrigiert). Druckerauswahl bleibt wie gehabt auf die erlaubten Klassen-Drucker vorausgewählt (`openPrintDialog`, unverändert) — für „Erstellen" (ohne Druck) ist die Auswahl irrelevant und wird nicht geprüft. | `tests/test_scan_station.py` (`test_activate_station_student_marks_active_without_printing`, `test_activate_station_student_reprint_keeps_baseline`, `test_scan_station_activate_endpoint` — bestätigt auch, dass KEIN `PrintJob` in die Warteschlange geht, `test_scan_station_activate_for_unknown_student_404`) + `uv run pytest` (562 Tests) + `uvx ruff check server/ automation/ tests/` + `node --check web/*.js` + Playwright-Screenshot des Drei-Knopf-Dialogs | 2026-08-12 | **562 Tests grün** (558 → 562), Ruff und JavaScript-Syntaxprüfung grün. **Browser-Livecheck offen** (s. unten) — insbesondere die Drei-Knopf-Optik im echten Theme (hell/dunkel) und dass „Erstellen" wirklich keinen Druckauftrag auslöst.

| V73 | **Zettel-Nachdruck: nur Drucken/Abbrechen im „Aktuell in Ausgabe"-Kästchen:** Der Nachdruck-Dialog dort (`opts.reprint: true`) zeigt nur noch „Drucken"/„Abbrechen" — kein „Erstellen"-Knopf, da der Zettel-Code dort schon existiert. Der Pairing-Kasten (`opts.reprint: false`) bleibt bei den drei Knöpfen aus V72. Kurzzeitig zog der Server dabei bei jedem Aufruf zusätzlich einen frischen Zettel-Code (`AppState.reallocate_station_code()`) — das wurde in V74 auf Niklas' Wunsch wieder zurückgenommen: der Code bleibt stabil, nur die Bücherliste aktualisiert sich pro Druck (war ohnehin schon immer so). | `tests/test_scan_station.py` — Zwei-Knopf-Dialog per Playwright-Screenshot gegengeprüft | 2026-08-12 | Ruff und JavaScript-Syntaxprüfung grün. Code-Verhalten s. V74. **Browser-Livecheck offen** (s. unten).
...
| V82 | **Lehreransicht: Offene Leihschein-Eingänge zuerst:** Innerhalb von `done` stehen Schüler mit aktivem, noch nicht angeklicktem „Leihschein entgegengenommen"-Button vor bereits angeklickten/erledigten Eingängen; danach Nachname/Vorname/Schüler-ID. | `node --check web/teacher.js` + `git diff --check` | 2026-08-12 | Syntax- und Diff-Prüfung grün. Echter Touchscreen-/Lehrerhandy-Livecheck bleibt offen.

| V81 | **Lehreransicht: Standardsortierung nach Status:** Die Liste startet mit „Nach Status" statt „Alphabetisch". Reihenfolge: `active -> pending -> absent -> done -> skipped`; innerhalb einer Gruppe Nachname/Vorname/Schüler-ID. Alphabetisch bleibt auswählbar. | `node --check web/teacher.js` + `git diff --check` | 2026-08-12 | Syntax- und Diff-Prüfung grün. Echter Touchscreen-/Lehrerhandy-Livecheck bleibt offen.

| V80 | **Lehreransicht: Host-übersprungene und abwesende Schüler getrennt behandeln:** `web/teacher.js` zeigt „Übersprungen" für `skipped` ohne Rücknahme-Button; nur `absent` erhält „Nicht abwesend". Die `auto_skipped`-Abbildung bleibt aktionslos. | `tests/test_teacher.py` (Rücknahme-Gate für `skipped` und `absent`) + `uv run pytest -q` + `uvx ruff check server/ automation/ tests/` + `node --check web/teacher.js` + `git diff --check` | 2026-08-12 | **586 Tests grün**, Ruff clean, JavaScript-Syntaxprüfung und Diff-Prüfung grün. Echter Touchscreen-/Lehrerhandy-Livecheck bleibt offen.
...
| V84 | **Geräte-Persistenz: Verwerfungsregel „nie verbunden" erst ab 5 min Serverlaufzeit:** Regel (2) aus V77 (nie per WS verbundene Helfer/Drucker-Displays/Scan-Stationen werden nicht gespeichert) griff bisher unabhängig von der Laufzeit — ein kurzer Neustart (Fehlstart, Konfig-Änderung, schnelles Durchstarten beim Aufbau) löschte damit Geräte, die gar keine Zeit zum Reconnect hatten, endgültig. Neu gated über `AppState.started_at_monotonic` (monotone Startzeit, in `app.lifespan` gesetzt statt nur im Konstruktor, damit ein früh erzeugter State die Uhr nicht vorzieht) und `sessions.PRUNE_MIN_UPTIME_S = 300.0`; `sessions._prunes_unconnected(state)` kapselt den Vergleich, alle drei `persist_*` filtern `connected_since_start or not prune`. Unter der Schwelle wird alles weitergeschrieben. Unverändert: IP-Fingerprint (Regel 1) und die `authorized`-Bedingung bei Displays/Stationen. | `tests/test_device_persistence.py` (bestehende „skips_never_connected"-Tests simulieren über `_long_run` jetzt einen langen Lauf; +3 Kurzlauf-Tests für Helfer/Displays/Stationen, +1 Grenzwert-Test `>=` bei exakt 300 s) + `uv run pytest` (590 Tests) + `uvx ruff check server/ tests/` | 2026-08-12 | **590 Tests grün** (586 → 590), Ruff clean. **Live-Check (echter Kurz-Neustart unter 5 min mit nie verbundenem Gerät) noch offen** (s. unten).
...
| V85 | **Scan-Station-Zettel: Feinschliff Layout, Sortierung und Dateiname:** `server/scan_station.py::build_sheet_pdf` — Abhak-Kästchen-Rect nachjustiert, bis Ober-/Unterkante mit der Textzeile fluchten (per gerenderten PyMuPDF-Pixmap-Crops visuell verifiziert, kein Browser nötig). „Bereits ausgeliehen" erscheint nur noch, wenn der Schüler wirklich schon etwas hat (`hide_if_empty=True`); „Noch vorgemerkt" behält seinen „—"-Platzhalter bei leerer Liste. Neue nummerierte Ablauf-Erklärung „So gehst du vor:" unter den Listen (4 Schritte, Leerzeile dazwischen, mehr Abstand zu den Listen). `print_station_sheet_for` (`server/sessions.py`) sortiert „Noch vorgemerkt" jetzt nach der klassenweiten Bücherliste-Reihenfolge (`get_book_order_for_form`, wie `web/common.js::renderBookRows`) statt alphabetisch — „Bereits ausgeliehen" (Ausgabezeitpunkt absteigend) und die Bestand-leer-/Ausgeblendet-Filterung waren bereits korrekt, keine Änderung nötig. Neue `_station_sheet_filename()`/`_station_sheet_label()` (analog `_own_slip_filename`/`_slip_print_label`) liefern `Scanstation <Klasse> <Nachname>, <Vorname>` als Download-/Druckjob-/Dateiname statt des generischen `scan_station_<student_id>`; `_download_slip_to_host()` bekam dafür einen optionalen `filename`-Parameter. | Bestehende `tests/test_scan_station.py`/`tests/test_sessions.py`/`tests/test_ws_scan_station.py` weiterhin grün (keine neuen automatisierten Fälle für die reine Layout-/Sortier-/Namensänderung — Verifikation lief über gerenderte PDF-Textextraktion und Pixmap-Crops in der Session) + `uv run pytest` (606 Tests) | 2026-08-13 | **606 Tests grün**, keine Regression. **Browser-/Drucker-Livecheck des Zettel-Layouts und Downloads mit echtem Klasse-/Namenskontext bleibt offen** (s. unten).
| V86 | **Host: Druckbutton bleibt ab Druckermodus sichtbar + Nachdruck als Host-Auftrag:** Der Druckbutton in der Schüler-Kachel „Aktuell in Ausgabe" (`web/host-render.js::renderCtxNowServing`, `studentClientPrint`) erscheint weiter erst mit Erreichen des Druckmodus, bleibt danach aber bis zum Abschluss sichtbar — auch während/nach dem Druck und im Unterschriften-Modus (links neben den rechtsbündigen Unterschrift-Button); bisher wurde er bei `slip_printing`/`slip_printed` ausgeblendet. Ein Klick NACH dem ersten erfolgreichen Druck legt den Auftrag in `routes/slips.py::print_loan_slip` immer als Host-Auftrag an (`role="host"`, `host_sid`, kein `student_token`), unabhängig davon, wer den ersten Druck ausgelöst hat (Host-Betreuerauslöser, Schüler-Selbstauslöser oder Scan-Station) und unabhängig vom `slip_trigger` — umgesetzt über `is_reprint` (`slip_printed`), `student_client` greift nur noch für den Erstdruck. `_mark_slip_printed` ist idempotent (Leihschein-Marker verändert sich durch den Nachdruck nicht); ein laufender Erstdruck bleibt über `in_flight_student_ids()` doppelt blockiert. | `tests/test_print_loan_slip_route.py` — alter „rejects when already printed"-Test ersetzt durch zwei Reprint-als-Host-Job-Tests (helper + auto trigger) plus In-Flight-Block-Test + `uv run pytest` (609 Tests) + `uvx ruff check server/ tests/` | 2026-08-13 | **609 Tests grün** (606 → 609), Ruff clean. **Browser-Livecheck offen** (s. unten) — Druckbutton-Sichtbarkeit über den Druck-/Unterschriften-Verlauf hinweg und Nachdruck am echten Drucker.
| V90 | **Drucker-Scanner: Klassenpräfix, Beep, 10s-Kamerapause, Scan-Station-Auto-Fertig.** Scanner-Kästchen zeigt die Klasse jetzt ohne „Klasse "-Präfix (Mirror `print_queue.slip_name`, war vorher inkonsistent). Drucker-Scanner spielt bei jedem erkannten Code sofort einen Beep (`Beeper.playBeep()`, Mirror Drucker-Display-Fertigton). Anzeigedauer am Drucker-Display + Kamera-Cooldown am Scanner von 5s auf 10s erhöht — die Kamera pausiert währenddessen komplett (`html5QrCode.pause(true)`/`.resume()`) statt nur erkannte Codes zu ignorieren. Größere Nachbesserung: Scan-Station-Schüler (kein Modus-B-Session, also keine „Leihschein erhalten"-Bestätigung möglich) wechseln jetzt SOFORT bei Druckende automatisch weiter — in den Unterschriften-Modus (`done_signed`) oder direkt auf „abgeschlossen" (Mirror des Modus-B-Zweigs in `confirm_slip_received`, nur ohne auf eine nie kommende Bestätigung zu warten); Erkennung über einen vergebenen Zettel-Code, `_mark_slip_printed` in `server/sessions.py`, greift unabhängig vom auslösenden `slip_trigger`. | `tests/test_scan_station.py` (3 neue Tests: Unterschriften-Modus, Auto-Fertig, Nicht-Station-Schüler unberührt) + `tests/test_ws_drucker_scan.py` (Klassenpräfix-Assertion) + `uv run pytest` (635 Tests) + `uvx ruff check server/ automation/ tests/` | 2026-08-13 | **635 Tests grün** (632 → 635; +3 neue Tests), Ruff clean. **Kein Browser-/Hardware-Livecheck** (Beep, Kamerapause, visuelle Timings) — s. unten.
| V89 | **Drucker-Scanner-Nachbesserungen: Reise-Animation, Doppel-Scan-Blitzer, kombinierte Reihenfolge.** „Zettel entsorgen"-Zeile im `ready`-Fall (immer, auch ohne Unterschreiben-Hinweis). Frisch erzeugte Aufträge: dasselbe Namens-Kästchen „reist" nach 5s per FLIP von der Scanner-Karte an seine echte Position in Warteschlange/Drucker (neue `PrintQueue.active_job_id_for_student`, `job_id` im `ready`-Payload, client-seitiges `readyTravelJobIds`-Unterdrücken + `revertScannerAndFlip` mit Seed-Rect). Doppel-Scan (`already`): kein Reise-Effekt, stattdessen einmaliger gelber Umrandungs-Blitzer am bereits existierenden Kästchen (`markFlaggedJob`/`.dd-order-flagged`). Host: Drucker+Scanner jetzt in EINER gemeinsamen, frei durcheinander verschiebbaren Box-Reihe (`PrinterDisplaySession.item_order`, `AppState._ordered_display_items`, `/api/drucker-display/reorder-items`) — dieselbe Reihenfolge bestimmt jetzt auch die Kartenreihenfolge am physischen Drucker-Display (`view["card_order"]`). | `tests/test_print_queue.py::test_active_job_id_for_student` + `tests/test_ws_drucker_scan.py` (job_id-Assertions) + `tests/test_drucker_display.py` (reorder-items, card_order-Interleaving, stabiles Anhängen) + `tests/test_device_persistence.py` (item_order-Roundtrip) + `uv run pytest` (632 Tests) + `uvx ruff check server/ automation/ tests/` | 2026-08-13 | **632 Tests grün** (626 → 632; +6 neue Tests), Ruff clean. **Kein Browser-/Hardware-Livecheck** für die visuellen Effekte (Reise-Animation, gelber Blitzer, Drag zwischen Drucker/Scanner-Boxen, Kartenreihenfolge am physischen Display) — s. unten.
| V88 | **Drucker-Scanner (`/drucker-scan`): neues Gerät für Schülerauslöser-Druck an der Scan-Station.** Pairing-Lebenszyklus (`PrinterScannerSession`, `ws_drucker_scan`), Persistenz-Roundtrip (`printer_scanner_store.py`, `assigned_scanner_ids` auf `PrinterDisplaySession`), alle sechs Scan-Klassifikationszweige (ready/already×3/pending_books/unknown, inkl. Buchcode-Auflösung über `loaned_to_id`) sowie die `print_mode`-Aufteilung „student" (mit erreichbarem Scanner → `scanner_names` statt Auftrag; ohne → identischer Fallback wie „Automatisch") sind per Unit-/WS-Tests abgesichert. Drucker-Display-Scanner-Karte (sechs Zustände, FLIP-Animation), Host-Verwaltungsreiter „Scanner" und die Display-Zuordnungs-Box-Grid sind **nur clientseitig** gebaut, ohne Browser-Test. | `tests/test_ws_drucker_scan.py` (14 neue Tests) + `tests/test_device_persistence.py` (Roundtrip) + `uv run pytest` (626 Tests) + `uvx ruff check server/ automation/ tests/` | 2026-08-13 | **626 Tests grün** (610 → 626; +16 neue Tests: 14 in `test_ws_drucker_scan.py` + 2 in `test_device_persistence.py`), Ruff clean. Löst die in V87 vermerkten 2 Failures auf (`test_state_contract`/`test_device_persistence` sind jetzt an das neue `printer_scanners`-Snapshot-Feld bzw. `assigned_scanner_ids` angepasst). **Browser-/Hardware-Livecheck offen** (s. unten) — Scanner-Karte am echten Drucker-Display, Host-Reiter, Kamera/Manuell-Umschaltung am echten Gerät.
| V87 | **Scan-Station meldet Schüler auch beim Abschließen/Überspringen ab:** `end_student` (`server/sessions.py`) löste die Scan-Station-Bindung bisher nur beim Trennen/Reset (`queue_status == "pending"`, inkl. Zettel-Code-Entwertung). Beim Abschließen/Überspringen (`done`/`skipped`/`absent`) blieb der Schüler stale auf der Station stehen. Jetzt löst jeder finale Durchlauf die Bindung via `release_station_student` (derselbe Pfad wie beim Trennen → `released`-Event → Station fällt auf „Zettel-Code scannen" zurück, `ready`). Der Zettel-Code wird dabei bewusst NICHT entwertet: ein Re-Scan wird via `resolve_station_code` am `done`-Status ohnehin abgelehnt („Vorgang bereits abgeschlossen."), und nach einem Reset-Queue (`done → pending`) bleibt der alte Zettel ohne Neudruck nutzbar. Greift für Host-Abschluss (`/api/finish`, `/api/finish-signed`), Helfer-Unterschriftsbutton, Auto-Abschluss nach Druck und Selbst-Abschluss (an der Station kann kein Handy-Selbstabschluss stehen, s. `resolve_station_code`). | `tests/test_scan_station.py::test_end_student_done_releases_station` + `uv run pytest` (610 Tests) + `uvx ruff check server/ tests/` | 2026-08-13 | **610 Tests, 608 grün** (609 → 610; +1 neuer Test). **2 vorbestehende, unabhängige Failures** (`test_state_contract::test_state_snapshot_key_set_is_stable`, `test_device_persistence::test_printer_display_store_load_rejects_ip_mismatch`) aus der laufenden `printer_scanners`-Arbeit in `state.py`/Persistenz — auf `origin/main` ohne die uncommitterten Änderungen laufen beide grün, nicht durch diesen Eintrag verursacht. Eigene Änderung + angrenzende Suiten (`test_scan_station`, `test_queue_flow`, `test_ws_scanner`, `test_scanner_reconnect`, `test_sessions`) grün. Ruff clean. **Browser-Livecheck offen** (s. unten) — Station fällt beim Host-Abschluss auf „Zettel-Code scannen" zurück.

## Offen / zu testen

### Offen 2026-08-13 (Drucker-Scanner: Live-Check am echten Gerät)

Pairing, Klassifikationslogik und `print_mode`-Aufteilung sind per Unit-/
WS-Tests abgesichert (`tests/test_ws_drucker_scan.py`,
`tests/test_device_persistence.py`), aber **nie mit einer echten Kamera, einem
echten Drucker-Display-Bildschirm oder im Schul-WLAN** durchgespielt. Offen:

- [ ] Drucker-Scanner öffnen (`/drucker-scan`) → Registrierungscode, am Host
      im „Scanner"-Reiter einschalten, im „Displays"-Reiter einem Drucker-
      Display zuordnen → Scanner-Karte erscheint dort mit korrektem Namen.
- [ ] Klasse mit `slip_trigger="student"`, Scan-Station-Schüler scannt letztes
      Buch → Station zeigt „Bitte scanne deinen Schülercode an <Name>…" mit
      korrekter Komma-/„oder"-Aufzählung bei mehreren erreichbaren Scannern.
- [ ] Schülercode am Drucker-Scanner scannen (Kamera UND manuelles Feld,
      Umschaltung über den Host-Switch) → sofortiger Beep, Scanner-Karte
      zeigt „`<Code>` wird geprüft", dann Klasse (ohne „Klasse "-Präfix,
      nur z. B. „10a") + Name mit „Wartet jetzt auf Druck" darüber, Box
      fährt sichtbar nach unten (FLIP), darunter (falls „Unterschreiben"
      aktiv) der Unterschreiben-Hinweis und IMMER „Du kannst den Zettel mit
      dem Schülercode jetzt entsorgen." in eigener Zeile.
- [ ] Nach dem Scan pausiert die Kamera am Drucker-Scanner für 10s (kein
      Decoding, kein Videobild) — ein zweiter Schüler kann in dieser Zeit
      nicht versehentlich seinen Code hineinscannen; danach läuft sie
      automatisch wieder.
- [ ] Nach 10s: dasselbe Namens-Kästchen „reist" sichtbar (FLIP, kein neues
      Kästchen) von der Scanner-Karte an seine echte Position in der
      zentralen Warteschlange bzw. auf dem Drucker, der ihn annimmt.
- [ ] Denselben Code erneut scannen, während der Auftrag wartet/druckt/schon
      gedruckt ist → Scanner-Karte zeigt passendes Schritt-Label +
      „bereits aufgegeben"-Hinweis (bleibt an der Scanner-Karte, KEIN Reise-
      Effekt); GLEICHZEITIG blitzt das bereits existierende Kästchen in
      Warteschlange/Drucker-Karte einmal gelb umrandet auf (Mirror des
      grünen „fertig"-Blitzers).
- [ ] Schüler mit noch offenen vorgemerkten Büchern scannen → Name + „Du hast
      noch nicht alle Bücher ausgeliehen…", KEINE Bewegung/Schritt-Label.
- [ ] Unlesbaren/fremden Code scannen → „Code unbekannt" + Betreuer-Hinweis.
- [ ] Kein Drucker-Scanner für die Klasse erreichbar → Station verhält sich
      wie „Automatisch" (sofortiger Druck), `station_print_needs_host`-
      Hinweistext erscheint bei Bedarf in der Schülerkartei.
- [ ] Sobald der Leihschein eines Scan-Station-Schülers physisch fertig
      gedruckt ist (egal ob Automatisch/Betreuer-/Schülerauslöser über den
      Drucker-Scanner): bei aktivem „Leihschein unterschreiben" wechselt er
      SOFORT (ohne Host-Klick) in den Unterschriften-Modus (Now-Serving zeigt
      den „Fertig unterschrieben"-Button); ohne Unterschreiben-Option schließt
      er sich SOFORT selbst ab (Status „abgeschlossen", ohne Host-Klick).
- [ ] Mehrere Drucker-Displays zeigen denselben Scanner (Mehrfachzuordnung) →
      alle betroffenen Displays aktualisieren die Scanner-Karte synchron.
- [ ] Host, Reiter „Displays": Drucker- und Scanner-Boxen stehen in EINER
      Reihe („Drucker und Scanner (N)"), per Drag beliebig durcheinander
      verschiebbar (auch Drucker↔Scanner) — die gewählte Reihenfolge
      bestimmt live die Spaltenreihenfolge am physischen Drucker-Display.
- [ ] Ein neu zugewiesener Drucker/Scanner (noch nicht in der Reihenfolge
      einsortiert) hängt beim ersten Erscheinen ans Ende an, statt die
      bestehende Reihenfolge zu stören.
- [ ] Ein Drucker-Scanner überlebt einen Serverneustart (Persistenz,
      `assigned_scanner_ids` bleibt am Display erhalten).

### Offen 2026-08-13 (Scan-Station: Abmeldung beim Abschluss: Live-Check)

Serverseitig abgesichert via `tests/test_scan_station.py::
test_end_student_done_releases_station`, aber **nicht im echten Browser-Paar**
(Host + Scan-Station) durchgespielt. Offen:

- [ ] Schüler an Scan-Station angemeldet (Bücherliste steht) → Host klickt
      „Abschließen" → Station fällt sofort auf „Bitte Schülerbarcode scannen"
      zurück (`released` → `ready`), keine stale Bücherliste.
- [ ] Gleiches beim Überspringen (`/api/skip`) und beim Auto-Abschluss nach
      Druck/Unterschrift.
- [ ] Zettel-Code des abgeschlossenen Schülers danach erneut scannen →
      „Vorgang bereits abgeschlossen." (Code bewusst nicht entwertet).
- [ ] Nach Reset-Queue denselben Zettel-Code scannen → Schüler lädt wieder
      (Code noch nutzbar, kein Neudruck nötig).

### Offen 2026-08-13 (Host-Druckbutton persistiert + Nachdruck: Live-Check)

Serverseitige Nachdruck-als-Host-Auftrag-Logik und In-Flight-Block sind per
Unit-Tests abgesichert (`tests/test_print_loan_slip_route.py`), aber **nicht
im echten Host-Browser** durchgespielt. Offen:

- [ ] Schülerclient (Modus B, `slip_trigger` „Betreuerauslöser") im Druckmodus:
      Druckbutton erscheint, nach dem Drucken bleibt er sichtbar (statt zu
      verschwinden) — auch nach „Leihschein erhalten"/Wechsel in den
      Unterschriften-Modus steht er links neben dem rechtsbündigen
      Unterschrift-Button.
- [ ] Auf den stehengebliebenen Button klicken → zweiter Leihschein druckt;
      Auftrag erscheint auf dem Drucker-Display als „Host"-Auftrag (ohne
      Helfername), Schülerclient zeigt keinen zweiten Progress (Nachdruck
      läuft vom Host).
- [ ] Gleiches für einen Scan-Station-Schüler mit `station_print_needs_host`
      (kein sichtbarer Drucker beim Druckmodus-Eintritt): Button druckt
      normal, bleibt sichtbar, Nachdruck geht ebenfalls als Host-Auftrag.
- [ ] Während der erste Druck noch läuft (`slip_printing`) ein zweiter Klick
      → Server weist mit 409 ab (Toast „Leihschein bereits gesendet oder
      nicht bereit"), kein zweiter physischer Auftrag.

### Offen 2026-08-13 (Scan-Station-Druckermodus: Live-Check)

Gate-Logik, Warteschlangen-Verhalten, Snapshot-Felder und die Buch-vs-
ungültig-Unterscheidung vor der Anmeldung sind per Unit-Tests abgesichert
(`tests/test_print_queue.py`, `tests/test_sessions.py`,
`tests/test_ws_scan_station.py`), aber **nie mit echten Drucker-Displays im
Schul-WLAN** durchgespielt. Offen:

- [ ] Ein erlaubter Drucker ist auf einem Display sichtbar, Klasse steht auf
      „Automatisch" — letztes Buch scannen → Station wechselt in den
      Druckermodus, Auftrag wird sofort dispatcht, Statustext nennt die
      richtige Anzahl Druckeranzeigen (Singular bei genau einer).
- [ ] Kein Display zeigt einen erlaubten Drucker, BEVOR der letzte Scan
      erfolgt — Station zeigt „bitte beim Host melden", am Host erscheint
      **kein** gelber Hinweis, aber der normale Druckbutton in „Aktuell in
      Ausgabe" ist klickbar und druckt normal.
- [ ] Auftrag ist bereits erzeugt (wartet), dann wird das letzte passende
      Display getrennt/umkonfiguriert — Host zeigt jetzt den gelben Hinweis
      + Drucker-Auswahlmenü; „Druckauftrag aktualisieren" befördert den
      Auftrag sichtbar in die Host-Rangposition und druckt ihn.
- [ ] Derselbe pausierte Auftrag: das Display kommt von selbst zurück (neu
      verbunden oder Drucker wieder zugewiesen) — der Auftrag wird OHNE
      Host-Eingriff automatisch dispatcht, der gelbe Hinweis verschwindet.
- [ ] Klasse mit „Leihschein unterschreiben" aktiv, Empfänger einmal
      „Betreuer" und einmal „Lehrer" — Statuszeile zeigt nach Leerzeile den
      passenden Satz.
- [ ] Vor der Anmeldung einen echten Buch-Barcode scannen (gelb) und einen
      erfundenen/unlesbaren Code (rot) — Wortlaut und Farbe stimmen.
- [ ] 30-s-Timer läuft per Stoppuhr korrekt ab und meldet unconditional ab,
      auch während ein Druckauftrag noch läuft; Fokus springt danach im
...
- [ ] Ein Display/eine Station freischalten, aber während des gesamten
      ...
      Shutdown-Hook in `app.py`). Voraussetzung: Der Lauf muss
      **länger als 5 min** gedauert haben (`sessions.PRUNE_MIN_UPTIME_S`).
- [ ] Gegenprobe zur 5-min-Schwelle: Gerät freischalten/wiederherstellen,
      Server **unter 5 min** laufen lassen und ohne Geräteverbindung neu
      starten — der Eintrag ist danach noch da (wird nicht verworfen).
...
Logik (`print_job_states` waiting/printing, `slip_status` im Snapshot,
Host-Badges wartet/druckt/gedruckt/Unterschrift, Helfer-Druckersymbol,
`slip_signing` live per `broadcast_host` in `confirm_slip_received`) ist per
Unit-Tests abgesichert (`tests/test_print_queue.py`). Am echten Gerät noch
offen (read-only, kein Buchungstest):

- [ ] Host-Queue zeigt nacheinander `Leihschein wartet` → `Leihschein druckt`
      → `Leihschein gedruckt`, ohne Reload.
- [ ] Mit aktiver „Leihschein unterschreiben"-Klassenoption wechselt der Host
      nach „Leihschein erhalten" live auf `Unterschrift` (kein Reload nötig).
- [ ] Helfer-Client zeigt das Druckersymbol für wartet/druckt/gedruckt und
      kein Druckersymbol mehr, sobald der „Leihschein unterschreiben"-Button
      erscheint.
- [ ] Bei `Fertig` (Session geschlossen) weder X/Y- noch Druckersymbol im
      Helfer-Client (nur Info-Badges bleiben).
...
Statusübergänge sind per Unit-/HTTP-/WS-Suite abgesichert (V80); reiner
...
- [ ] Ein vom Host übersprungener Schüler erscheint in der Lehreransicht als
      „Übersprungen" und ohne „Nicht abwesend"-Button; nur ein `absent`-
      Schüler bekommt diesen Button. Die `auto_skipped`-Abbildung bleibt
      ebenfalls aktionslos.
...
- [ ] **Wisch-Geste (V42):** auf einem echten Touchscreen einen wartenden
      ...
      schnellt die Zeile zurück, oberhalb löst sie aus (Status „Abwesend").
...
### Offen 2026-08-11 (Scan-Station)

- [ ] **Echter Barcode-Livecheck:** Zettel ausdrucken und mit der Handy-/
      Stations-Kamera (html5-qrcode/ZXing) einlesen. Der Code-39-Encoder ist
      gegen die Standardtabelle geprüft und die Maße im PDF sind nachgemessen,
      aber **noch kein einziger physisch gedruckter Barcode wurde gescannt** —
      Papierweiß, Druckerauflösung und Kameraabstand entscheiden hier.
      Fällt die Lesequote ab: Balken verbreitern (`_WIDE_RATIO`) bzw. den
      Barcode höher setzen.
- [ ] **Ende-zu-Ende am Gerät:** Station öffnen → am Host freischalten →
      Zettel drucken → Code scannen → Bücher scannen → 30 s warten →
      Rückfall auf „Zettel-Code scannen". Mit ausgemusterten Büchern und
      `ALLOW_BOOKING=false`.
- [ ] **Zwei Schüler nacheinander an derselben Station** (Freigabe per „Fertig"
      und per Timeout) — Bücherliste des Vorgängers darf nicht stehen bleiben.
- [ ] **Nachdruck trägt denselben Code** am echten Drucker verifizieren
      (im Test nur auf State-Ebene geprüft).
- [ ] **Rangfolge unter Last:** Helfer, Station und Handy-Client gleichzeitig
      auf einen 1er-Pool (`WORKER_CONTEXTS=1`) — Helfer muss zuerst drankommen,
      die Wartenden sollen ihre Position sehen. Bisher nur auf Vergabe-Ebene
      getestet, nicht mit echten Playwright-Contexts.
- [ ] **Drucker-Display-Anzeige:** Zetteldruck erscheint mit Klasse/Name und
      Host-Symbol, und der Leihschein-Status des Schülers bleibt danach
      unverändert (im Test nur die Server-Seite abgesichert).
- [ ] **Schülerclient nach dem CSS-Herausziehen im Browser prüfen:** Die
      Scan-Ansicht von `student.html` bezieht ihr Aussehen jetzt aus
      `web/scan-view.css`. Automatisch geprüft ist nur, dass keine benutzte
      CSS-Klasse ohne Regel dasteht — das tatsächliche Layout (Pairing-Ansicht,
      Scan-Ansicht, Druck-/Unterschreiben-/Abschluss-Ansicht) wurde **nicht**
      in einem Browser angesehen.
- [ ] **Manueller Modus an der Station mit echtem Handscanner:** Eingabefeld
      behält den Fokus, Enter löst den Scan aus, Fokus-Warnbanner erscheint
      beim Fokusverlust, Umschalten am Host wirkt sofort auf dem Gerät.
- [ ] **Blockierendes Modal an der Station live:** ausgemustertes Buch
      scannen → Kamera UND manuelles Feld tatsächlich gesperrt (nicht nur im
      Test simuliert); Host klickt „Schließen" im Now-Serving-Kästchen →
      Station scannt sofort wieder.
- [ ] **Stationswechsel live mit zwei echten Zetteln:** Schüler B scannt
      seinen Zettel, während Schüler A noch an der Station ist — Übergang
      soll sich für den wartenden Schüler nicht wie ein Fehler anfühlen (kurzer
      „Zettel-Code scannen"-Blitz zwischen den beiden ist erwartet, s.
      `release_station_student`s `released`-Frame).
- [ ] **Host-Queue-Status live prüfen:** Zettel drucken → Schüler wird sofort
      „Aktiv"/„Bücher sammeln" in der Klassen-Queue und im Now-Serving-Kästchen
      (falls dort sichtbar); an der Station anmelden → zweizeilige X/Y-Anzeige
      mit Host-Symbol + Stationsname; abmelden → zurück zu „Bücher sammeln";
      alle Bücher ausleihen und abmelden → bleibt bei der Zahl stehen statt
      zurückzufallen. Bisher nur auf Datenebene getestet, nicht im Browser
      angesehen.
- [ ] **Now-Serving-Stationszeile live prüfen:** Layout (Zettel-Code rechts,
      Stationsname+Symbol links, beide korrekt grau/blau eingefärbt wie
      Klasse/Status), der kleine Stations-Trennen-Knopf erscheint nur bei
      aktiver Anmeldung und meldet sofort ab (Station fällt auf „Zettel-Code
      scannen" zurück, der Schüler bleibt in der Queue aktiv). Bisher nur auf
      Datenebene getestet, nicht im Browser angesehen.
