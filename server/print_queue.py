...
class PrintJob:
    ...
    # WS-Ziel für print_progress/print_result. Auto-Fertig läuft erst nach der
    # „Leihschein erhalten"-Bestätigung serverseitig in
...
class PrintQueue:
    ...
    async def _notify_result_if(self, job: PrintJob | None) -> None:
        ...
        if job.student_token:
            # Session kann nach Auto-Fertig (`end_student` in
            # `sessions.confirm_slip_received`) bereits entwertet/entfernt
            # sein → still überspringen.
            session = state.student_sessions.get(job.student_token)
            if session is not None and session.ws is not None:
